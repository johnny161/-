...
"""
    ...
    Desc: 用户-电影打分预测
          CatBoost
          特点：自动处理组合类别特征
          使用 iteminfo.csv 和 userinfo.csv 的部分信息
    Score: 0.468 (五折精度=0.462)（random_state=1234,iter:10000,depth:6,lr:0.1)
    Time: 17s
    Date: 2020/8/5
"""
...
df_x_train = pd.read_csv('AIyanxishe_79/train.csv', delimiter=',', names=cols,
                    converters={'userId': str, 'movie_id': str, 'timestamp': str})
df_x_test = pd.read_csv('AIyanxishe_79/test.csv', delimiter=',', names=cols_test,
                    converters={'userId': str, 'movie_id': str, 'timestamp': str})

# 增加user和item的其他信息
userInfo = pd.read_csv('AIyanxishe_79/userInfo.csv', delimiter=',', converters={'userId': str, 'useGender': str})
itemInfo = pd.read_csv('AIyanxishe_79/itemInfo.csv', delimiter=',', converters={'movie_id': str})
# 性别属性数值化
gender_dict = {'M': 0, 'F': 1}
...
i_r = dict()
i_r_num = dict()
with open('AIyanxishe_79/train.csv', 'r') as csvfile:
...
index = 0
with open("./AIyanxishe_79/test.csv", 'r', newline='') as readfile:
    with open("./submit.csv", 'w', newline='') as writefile:
